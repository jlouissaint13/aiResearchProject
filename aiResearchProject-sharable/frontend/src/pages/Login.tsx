...
const Login = () => {
    ...
    const [guestEnabled, setGuestEnabled] = useState<boolean>(true);
    ...
    useEffect(() => {
        fetch("http://localhost:8000/admin/flags/guest_enabled/status")
            .then((res) => res.json())
            .then((data) => setGuestEnabled(Boolean(data?.guest_enabled)))
            .catch(() => setGuestEnabled(true)); // default to true if something fails
    }, []);
